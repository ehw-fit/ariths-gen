...
class UnsignedHanCarlsonAdder(ArithmeticCircuit):
    ...
    Han-Carlson adders are a family of tree adders that represent a tradeoff between Kogge-Stone and Brent-Kung implementations.
    ...
    Han-Carlson networks provide tradeoff between the number of wires, the number of stages in the PG logic and the number of Black cells used.
    ...
    Args:
        ...
        prefix (str, optional): Prefix name of unsigned hca. Defaults to "".
        name (str, optional): Name of unsigned hca. Defaults to "u_hca".
        config_choice (int, optional): Tradeoff implementation choice concerning the number of wires, the number of stages in the PG logic and the number of Black cells used. The number of choices goes from 1 up to ⌈log2(N)⌉-2 for N > 4, otherwise the choice is 1. Defaults to 1.
    """
    def __init__(self, a: Bus, b: Bus, prefix: str = "", name: str = "u_hca", config_choice: int = 1, **kwargs):
        ...
        for i_wire in range(self.N):
            ...
            # For all bit indexes expect for the last one, proceed with the parralel prefix PG logic
                ...
                if (i_wire+2) % config_offset == 0 or (i_wire < 4 and i_wire % 2 == 0):  # These input bits form a Kogge-Stone adder structure
...
class SignedHanCarlsonAdder(UnsignedHanCarlsonAdder, ArithmeticCircuit):
    ...
    Args:
        a (Bus): First input bus.
        b (Bus): Second input bus.
        prefix (str, optional): Prefix name of signed hca. Defaults to "".
        name (str, optional): Name of signed hca. Defaults to "s_hca".
        config_choice (int, optional): Tradeoff implementation choice concerning the number of wires, the number of stages in the PG logic and the number of Black cells used. The number of choices goes from 1 up to ⌈log2(N)⌉-2 for N > 4, otherwise the choice is 1. Defaults to 1.
    """
    def __init__(self, a: Bus, b: Bus, prefix: str = "", name: str = "s_hca", config_choice: int = 1, **kwargs):
